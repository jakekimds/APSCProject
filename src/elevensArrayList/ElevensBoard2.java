...
public class ElevensBoard2 extends Board {
	...
	public ElevensBoard2() {
		...
		System.out.println("Jake Kim ---  Period 4 --- 2018 March 28 --- CA-SU-F106-10");
	}
	...
	public boolean isLegal(List<Integer> selectedCards) {
		...
		if (containsTrioSum11(selectedCards) || containsFacePair(selectedCards)) {
			return true;
		}
		return false;
	}
	...
	public boolean anotherPlayIsPossible() {
		/* *** TO BE IMPLEMENTED IN ACTIVITY 9 *** */
		if (containsTrioSum11(cardIndexes()) || containsFacePair(cardIndexes())) {
			return true;
		}
		return false;
	}
		...
		for (int i = 0; i < selectedCards.size(); i++) {
			for (int j = i + 1; j < selectedCards.size(); j++) {
				for (int k = j + 1; k < selectedCards.size(); k++) {
					if (cardAt(selectedCards.get(i)).pointValue() + cardAt(selectedCards.get(j)).pointValue()
							+ cardAt(selectedCards.get(k)).pointValue() == 11) {
						return true;
					}
				}
			}
	...
	private boolean containsFacePair(List<Integer> selectedCards) {
		...
		for (int i = 0; i < selectedCards.size(); i++) {
			for (int j = i + 1; j < selectedCards.size(); j++) {
				if (cardAt(selectedCards.get(i)).pointValue() == 0 && cardAt(selectedCards.get(j)).pointValue() == 0) {
